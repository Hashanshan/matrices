    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="relative w-full max-w-md max-h-[90vh] bg-slate-200/30 backdrop-blur-3xl rounded-[2.5rem] p-6 sm:p-8 border border-white text-white shadow-[0_25px_70px_rgba(0,0,0,0.4)] overflow-y-auto scrollbar-none cursor-default"
        >
          {/* Close Button - iPad Style Pill */}
          <button
            onClick={onClose}
            className="absolute top-5 right-5 p-2.5 text-slate-300 hover:text-white hover:bg-white/10 rounded-full transition-all shadow-xs border border-white/20 cursor-pointer"
          >
            <X size={18} />
          </button>
          ...
          <div className="text-center mb-6">
            <div className="w-16 h-16 bg-[#0f172a] text-white rounded-full flex items-center justify-center mx-auto mb-4 shadow-xl shadow-slate-950/40 border border-white/20">
          ...
          {/* Error Message */}
          {errorMsg && (
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center text-xs font-black text-red-400 mb-4 uppercase">
              {errorMsg}
            </motion.p>
          )}
          ...
          {mode === 'pin' ? (
            <>
                ...
                {[0, 1, 2, 3].map(index => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={`w-12 h-12 rounded-full flex items-center justify-center border transition-all shadow-xs ${errorMsg
                        ? 'border-red-400 bg-red-500/30 text-red-300'
                        : isFilled
                          ? 'border-white/40 bg-[#0f172a] text-white shadow-md'
                          : 'border-white/10 bg-slate-400/40 text-transparent'
                        }`}
                    >
              ...
              <div className="grid grid-cols-3 gap-4 max-w-[260px] mx-auto mb-6">
                {['1', '2', '3', '4', '5', '6', '7', '8', '9'].map(num => (
                  <button
                    ...
                    className="w-16 h-16 rounded-full bg-slate-200 hover:bg-white text-[#0f172a] font-black text-2xl border border-slate-300 shadow-lg active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center mx-auto cursor-pointer"
                  >
                    {num}
                  </button>
                ))}
                <button
                  ...
                  className="w-16 h-16 rounded-full bg-slate-500/40 hover:bg-slate-500/60 text-slate-200 font-extrabold text-xs uppercase border border-white/20 disabled:opacity-30 transition-all flex items-center justify-center mx-auto shadow-xs cursor-pointer"
                >
                  CLEAR
                </button>
                <button
                ...
                <button
                  ...
                  disabled={isSubmitting || pin.length === 0}
                  className="w-16 h-16 rounded-full bg-slate-500/40 hover:bg-slate-500/60 text-slate-200 flex items-center justify-center border border-white/20 disabled:opacity-30 transition-all mx-auto shadow-xs cursor-pointer"
                >
                  <Delete size={20} />
                </button>
              </div>
              ...
              <button
                ...
                className="w-full bg-[#0f172a] hover:bg-[#1e293b] text-white py-4 rounded-full font-black text-sm uppercase tracking-wider disabled:opacity-40 transition-all shadow-xl border border-white/10 flex items-center justify-center gap-2 mb-3 cursor-pointer"
              >
                {isSubmitting ? (
