...
export async function GET(request: NextRequest) {
  try {
    ...
    const basePath = BACKEND_URL.endsWith('/api') ? '/catelogue/orders' : '/api/catelogue/orders';
    ...
    console.error('[API Proxy] Error fetching catalogue orders from backend:', err);
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('[API Proxy] Error posting catalogue orders from backend:', err);
    return NextResponse.json({ msg: 'Internal proxy error' }, { status: 500 });
  }
}
